...
export default function UserManagement() {
  ...
  const handleBatchStatus = async (toBeActive: boolean) => {
    ...
    try {
      ...
      // Update local state
      setUsers(prev => prev.map(u => selectedChurchIds.includes(String(u.id)) ? { ...u, isEnabled: toBeActive } : u));
      setChurchesBank(prev => prev.map(c => selectedChurchIds.includes(String(c.id)) ? { ...c, isEnabled: toBeActive } : c));
  ...
  const handleBatchReading = async (toBeAllowed: boolean) => {
    ...
    try {
      ...
      // Update local state
      setUsers(prev => prev.map(u => selectedChurchIds.includes(String(u.id)) ? { ...u, isAllowedToRead: toBeAllowed } : u));
      setChurchesBank(prev => prev.map(c => selectedChurchIds.includes(String(c.id)) ? { ...c, isAllowedToRead: toBeAllowed } : c));
      ...
      <div className="bg-slate-50 p-4 rounded-xl border border-slate-200/80 mb-6 flex flex-col gap-4 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            >
              {filteredUsers.length > 0 && filteredUsers.every(u => selectedChurchIds.includes(String(u.id))) ? (
                <>
            ...
            <motion.div
              ...
              className={`bg-white p-6 rounded-xl shadow-sm border transition-all relative ${
                isEditing === user.id 
                  ...
                  : selectedChurchIds.includes(String(user.id))
                ...
                <>
                  <div className="flex items-start gap-3">
                    ...
                    <button
                      ...
                      title={selectedChurchIds.includes(String(user.id)) ? "إلغاء التحديد المجمع" : "تحديد مجمع"}
                    >
                      {selectedChurchIds.includes(String(user.id)) ? (
