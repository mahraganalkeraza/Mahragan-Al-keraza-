...
export function getHourlyExamToken(offsetDays: number = 0): string {
  try {
    ...
    // Convert absolute system time to Cairo Time (UTC + 3 hours)
    // 3 * 60 * 60 * 1000 = 10800000 milliseconds
    const utc = now.getTime() + (now.getTimezoneOffset() * 60000);
    const cairoTime = new Date(utc + 10800000);

    const targetHour = 22; // 10:00 PM Rollover

    // If current Cairo hour is less than 22:00, the active block belongs to the previous calendar day
    if (cairoTime.getHours() < targetHour) {
      cairoTime.setDate(cairoTime.getDate() - 1);
    }

    // Apply secondary day offsets if checking previous cycles
    if (offsetDays !== 0) {
    ...
    // Unified date block strictly locked to the target hour (22)
...
export function validateHourlyExamToken(token: string | null): boolean {
  ...
  // Clean token input from any whitespaces
  const cleanToken = token.trim();
  
  // Active token for the current 24-hour window
  const currentToken = getHourlyExamToken(0);
  
  // Previous token window (offset -1 day) to handle mid-exam transitions safely
  const previousToken = getHourlyExamToken(-1);
  
  return cleanToken === currentToken || cleanToken === previousToken;
}
